...
\begin{itemize}
  \item address: contiene la struttura che rappresenta l'indirizzo di un nodo:

  \item raft\_log: viene definito il sistema di log di Raft, in cui la struttura LogInstance rappresenta una singola voce nel log di Raft. Il pacchetto importa anche una codifica basata su protobuf per serializzare i messaggi 
  ...
  \item rpcs: definisce un'interfaccia generale Rpc, utilizzata in tutto il progetto per astrarre la logica delle varie chiamate RPC nel sistema Raft. \\AppendEntryRpc implementa l'RPC per l'aggiunta delle voci di log. Questa struct 
